...
- 结果保留：`done`、`warning` 和 `failed` 会话支持单条删除，也可以在展开面板中一键清除。
- 应用内更新：macOS 使用 Sparkle、Windows 安装版使用 Velopack 自动检查、校验并一键升级。
...
Windows 推荐下载 `io.github.xiaoaozz.AgentPulse-Setup.exe`。安装后应用会常驻系统托盘，并支持应用内一键升级；随包 Hook 位于安装目录的 `Scripts/`。`AgentPulse-<version>-win-x64.zip` 继续作为便携版提供，但便携版不能在运行时覆盖自身，因此只会提示改用 Setup 安装版。

当前 `0.1.0` 以及更早版本尚未内置更新器，需要手动安装一次包含更新功能的新版本；之后即可在应用内完成升级。
...
dist/AgentPulse-0.1.0-macos-universal.zip.sha256
dist/AgentPulse-0.1.0-win-x64.zip
dist/AgentPulse-0.1.0-win-x64.zip.sha256
dist/velopack-win-x64/io.github.xiaoaozz.AgentPulse-Setup.exe
...
其中 `AgentPulseNotary` 是预先通过 `xcrun notarytool store-credentials` 保存的钥匙串配置。脚本会签名 App、提交 Apple 公证、装订公证票据并重新生成 ZIP 和 SHA-256 文件。
...
| `SPARKLE_PUBLIC_KEY` | Sparkle EdDSA 公钥，可以内置在 App 中 |
| `SPARKLE_PRIVATE_KEY` | Sparkle EdDSA 私钥，仅供 Release 工作流签名更新包 |
...
cd dist && shasum -a 256 -c AgentPulse-0.1.0-macos-universal.zip.sha256
```

### 配置应用内更新

macOS 更新由 Sparkle 2.9.2 提供。首次配置时，从 [Sparkle Release](https://github.com/sparkle-project/Sparkle/releases/tag/2.9.2) 下载官方工具，然后执行：

```bash
bin/generate_keys --account AgentPulse
bin/generate_keys --account AgentPulse -p
bin/generate_keys --account AgentPulse -x agentpulse-sparkle-private-key
```

将第二条命令输出保存为仓库 Secret `SPARKLE_PUBLIC_KEY`，将导出的私钥文件内容保存为 `SPARKLE_PRIVATE_KEY`，随后立即安全删除导出文件。私钥仍会保存在当前 macOS 用户钥匙串中；不要提交或发送给其他人。

Release 工作流会把公钥写入 App，使用私钥生成带 EdDSA 签名的 `appcast.xml`，并与 macOS ZIP 一起上传到 GitHub Release。应用默认自动检查更新，用户也可以在设置页点击“检查更新…”。

Windows 安装包使用 Velopack 1.2.0。`scripts/build-windows.ps1` 会生成 Setup、完整更新包和 `releases.win.json`；Windows 客户端从公开的 GitHub Releases 读取这些文件，下载完成后退出、替换并重新启动应用。
...
- 当前没有会话持久化、开机自启或 DMG 打包能力；macOS 正式版本支持签名与公证，macOS 和 Windows 安装版均支持应用内更新。
