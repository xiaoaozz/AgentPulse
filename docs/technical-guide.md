...
Windows 推荐下载 `io.github.xiaoaozz.AgentPulse-win-Setup.exe`。安装后应用会常驻系统托盘，随包 Hook 位于安装目录的 `Scripts/`。不想安装时可下载 `io.github.xiaoaozz.AgentPulse-win-Portable.zip`。新版本均从 Releases 手动下载安装。
...
dist/velopack-win-x64/io.github.xiaoaozz.AgentPulse-win-Portable.zip
