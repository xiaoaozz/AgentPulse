...
- macOS 原生悬浮球 / 刘海面板：默认不创建菜单栏或 Dock 图标，无需 Electron 常驻运行时。
- Windows 原生托盘：点击系统托盘图标后在任务栏右下角展开 WinUI 3 会话面板。
- 多会话聚合：同一 `session_id` 的事件会原地更新，并按处理优先级排序。
- 待操作提示：悬浮球与刘海面板都会优先展示等待操作的会话状态。
- 悬浮球：默认以 44 × 44 的轻量控件常驻，显示全局状态和进行中会话数；支持拖动并记住位置，点击后按最近使用时间倒序展示最多 5 个会话。
- 刘海面板：可在设置中切换，自动识别内置刘海屏幕，在刘海两侧显示英文状态和进行中会话数；稳定悬停后展开面板，避免指针经过屏幕顶部时误触。
...
`SessionEnd` 产生的 `offline` 只描述 Agent 进程已经离线，不会覆盖同一会话已有的 `paused`、`done`、`warning` 或 `failed` 任务结果。这样中止或完成后的记录仍保留原状态，并可正常单独删除或一键清理。
...
- 悬浮球可在屏幕间拖动并记住位置；展开时会自动保持在当前屏幕的可见区域内。
- 如果选择刘海模式但没有刘海屏幕，面板会回退到主屏幕顶部中央显示。
- 会话仅保存在内存中；退出或重启 AgentPulse 后不会恢复历史记录。

### 悬浮球

- 折叠状态占用 44 × 44，细环颜色表示当前最高优先级状态，角标显示进行中会话数。
- 点击悬浮球展开或收起最近会话；拖动悬浮球可以调整位置，应用重启后会恢复到上次位置。
- 展开面板右下角的收起按钮始终回到悬浮球；如果当前处于刘海模式，点击该按钮也会自动切换为悬浮球。
...
- 应用默认不显示 macOS 菜单栏或 Dock 图标；展开状态面板后可点击右下角电源图标安全退出。
